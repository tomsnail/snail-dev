...
//		}
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme){
		...
		model.put("functionAuthor", StringUtils.capitalize(genScheme.getFunctionAuthor()));
		...
		try {
			if(genScheme.getGenTable()!=null){
				model.put("tableName", StringUtils.capitalize(genScheme.getGenTable().getTableName()));
		...
		try {
			if(genScheme.getGenClass()!=null){
				model.put("className", StringUtils.uncapitalize(genScheme.getGenClass().getClassName()));
				model.put("ClassName", StringUtils.capitalize(genScheme.getGenClass().getClassName()));
				model.put("methods", genScheme.getGenClass().getColumnList());
				model.put("imports", genScheme.getGenClass().getImportList());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		model.put("artifactId", genScheme.getArtifactId());
		
		model.put("groupId", genScheme.getGroupId());
		
		model.put("partifactId", genScheme.getPartifactId());
		
		model.put("pomversion", genScheme.getPomversion());
		
		model.put("jdbcUrl", genScheme.getJdbcUrl());
		
		model.put("jdbcUsername", genScheme.getJdbcUsername());
		
		model.put("jdbcPassword", genScheme.getJdbcPassword());
		
		//nstr.replaceAll("\\.","point");
		
		model.put("packagePath", StringUtils.lowerCase(genScheme.getPackageName()).replaceAll("\\.","/"));
