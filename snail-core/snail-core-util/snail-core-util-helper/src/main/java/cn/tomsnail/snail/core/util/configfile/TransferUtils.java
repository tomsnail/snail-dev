...
public class TransferUtils {
    ...
    public static void properties2Yaml(String path) {
        ...
        try (InputStreamReader is = new InputStreamReader(new FileInputStream(path), Charset.forName(ENCODING))){
            parser = factory.createParser(is);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        try( OutputStreamWriter outputStreamWriter = new OutputStreamWriter(new FileOutputStream(path), Charset.forName(ENCODING))) {
            YAMLFactory yamlFactory = new YAMLFactory();
            YAMLGenerator generator = yamlFactory.createGenerator(outputStreamWriter);
