...
public class J2CacheConfig {
    ...
    public final static J2CacheConfig initFromConfig(Properties properties) {
        ...
        config.serialization = trim(config.properties.getProperty("system.cache.j2cache.serialization"));
        config.broadcast = trim(config.properties.getProperty("system.cache.j2cache.broadcast"));
        config.l1CacheName = trim(config.properties.getProperty("system.cache.j2cache.L1.provider_class"));
        config.l2CacheName = trim(config.properties.getProperty("system.cache.j2cache.L2.provider_class"));
        config.syncTtlToRedis = !"false".equalsIgnoreCase(trim(config.properties.getProperty("system.cache.j2cache.sync_ttl_to_redis")));
        config.defaultCacheNullObject = "true".equalsIgnoreCase(trim(config.properties.getProperty("system.cache.j2cache.default_cache_null_object")));

        String l2_config_section = trim(config.properties.getProperty("system.cache.j2cache.L2.config_section"));
        if (l2_config_section == null || "".equals(l2_config_section.trim())) {
            l2_config_section = config.l2CacheName;
        }

        config.broadcastProperties = config.getSubProperties("system.cache.j2cache."+config.broadcast);
        config.l1CacheProperties = config.getSubProperties("system.cache.j2cache."+config.l1CacheName);
        config.l2CacheProperties = config.getSubProperties("system.cache.j2cache."+l2_config_section);
        return config;
    }
