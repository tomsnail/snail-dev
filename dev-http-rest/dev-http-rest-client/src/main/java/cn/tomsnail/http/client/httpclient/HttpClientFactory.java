package cn.tomsnail.http.client.httpclient;
...
public class HttpClientFactory {
	 ...
	 * @exception no
	 */
	public static HttpClientFactory initialize(int MAX_TOTAL_CONNECTIONS,
			int MAX_ROUTE_CONNECTIONS, int WAIT_TIMEOUT) {
		synchronized (LOCK_BYTE) {
			if (factory == null)
				factory = new HttpClientFactory(MAX_TOTAL_CONNECTIONS,
						WAIT_TIMEOUT, MAX_ROUTE_CONNECTIONS, CONNECT_TIMEOUT,
						READ_TIMEOUT);
			return factory;
		}
	}
	...
	public ResultData<Map<String, Object>> post(String host, String url,
			RequestData<Map<String, Object>> requestData) {
		HttpClient httpClient = getHttpClient();
		httpClient.getParams().setParameter(
				ClientPNames.ALLOW_CIRCULAR_REDIRECTS, true);
		HttpPost httpost = new HttpPost(url); // 设置响应头信息
		...
		httpost.addHeader("Accept-Encoding", "gzip, deflate");
		String result = "";
		ResultData<Map<String, Object>> resultData = null;
		try {
			String params = "";
			if (requestData != null) {
				params = OBJECT_MAPPER.writeValueAsString(requestData);
			} else {
				return null;
			}
			StringEntity entity = new StringEntity(params.toString(), "UTF-8");
			...
			if (response != null) {
				...
				if (status >= 200 && status < 300) {
					result = EntityUtils
							.toString(response.getEntity(), "UTF-8");
					resultData = OBJECT_MAPPER.readValue(result,
							ResultData.class);
				} else {
				}
			}
	...
	private static HttpClientFactory httpClientFactory = HttpClientFactory
			.initialize();

	public static void main(String[] args) {
		// System.out.println(httpClientFactory.get(new
		// StringBuffer("http://www.baidu.com/s?wd=123")));
		long d1 = System.currentTimeMillis();
		for (int i = 0; i < 100; i++)
			System.out.println(httpClientFactory.post("",
					"http://192.168.169.54:8080/system/get/config",
					new RequestData<Map<String, Object>>()).getBody());
